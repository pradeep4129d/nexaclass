...
export const Test = () => {
  ...
  const handleTaskSubmit = async () => {
    ...
    if (testItem.includeEditor) {
      ...
      try {
        const response = await axios.post("http://localhost:3000/chatbot/evaluate", {
          questions: questionData
        });
        marksScored += response.data.totalMarks;
      } catch (error) {
  ...
  const updatedReport = {
    ...ActivityReport,
    marksScored,
    taskAnswers: JSON.stringify(taskAnswers),
    passed: (marksScored / 100) * 100 >= 65
  };
